...
import random # On importe le module random qui nous permet de rendre aléatoireledébut de chaque partie
import time
# Init
...
# window.minsize(w,h) # Si l'utilisateur veut que la taille de la fenêtre soit bloqué sur lemode plein écran. Problème : Cache un peu le boutton MENU
# Functions

def btn_maker(x,y): # Cette fonction permet de rendre la création de la grille de boutton plus compact
    return Button(
        table,
        width=width_button,
        ...
        command=lambda: click(x,y), # Création d'un boutton de notre grille (répété 9 fois)
    )

mode = 0 # Variable qui permet de rendre le boutton menu multitache, il ferme et ouvre le menu
def new_window(): # Fonction qui permetde crée ce nouveau menu et de le supprimer selon le mode
    ...
    else: # Si la Frameest présente sur la page
...
def reset(): # Partie assez redondante du code, qui a donc été transformé en fonction, permet de clear le tableau de jeu après victore ou nulle.
    global button_storage # Importation du tableau qui stock l'ensemble des bouttons
    
    for j in range(len(button_storage[0])): # # On boucle sur la première partie du tableau didimentionnel
        for k in range(len(button_storage[0][j])): # Sur la deuxième
            button_storage[0][j][k]["text"] = "" # Pour le boutton en question, son texte s'actualise sur "" (rien)
...
def checker(c): # Fonction qui permet d'annoncer le vainqueur, d'incrémenter le score de ce dernier et d'actualiser le tableau virtuel
...
def verification_win(board, winner_sign): # Fonction qui vérifie si un joueur a gagné
    ...
    elif board[0][0] == board[1][0] == board[2][0] != 0: # Collone de gauche
        checker(winner_sign)

    elif board[0][1] == board[1][1] == board[2][1] != 0: # Collone du milieu
        checker(winner_sign)

    elif board[0][2] == board[1][2] == board[2][2] != 0: # Collone de droite
        checker(winner_sign)
...
def poser(sign, x, y): # Fonction qui permet de placer les jetons sur les 2 tableaux (virtuels et celui des bouttons)
    ...
    if virtual_board[x][y]: # Si la case est déjà prise
        ...
        button_storage[0][x][y]["fg"] = "white" # On lui donne une couleure blanche (quand un texte est nouvellement ajouté sur tkinter, il faut lui donné une nouvelle couleure).
